...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception{
		...
		// 화면에서 오는 로그인 정보 수정
		http.csrf().disable();   //csrf() : 공격에 대한 설정 풀기
		
		// 권한 승인 요청
		// ADMIN > MANAGER > USER
		http.authorizeRequests()
			.antMatchers("/user/list").hasRole("ADMIN")  //antMatchers() : 접근을 허용하는 경로, hasRole('권한') : 해당 권한 확인
			.antMatchers("/", "/user/login", "/user/register", "/board/list", "/board/detail",
					"/upload/**", "/comment/**", "/resources/**").permitAll()  //permitAll() : 누구나 접근이 가능한 경로
			.anyRequest().authenticated();  //authenticated() : 인증된 사용자만 가능한 경로
			
		// 로그인 페이지 구성 : id=>email, pw=>pwd
		// Controller에 주소요청 맵핑은 같이 있어야함.(필수!)
		http.formLogin()
